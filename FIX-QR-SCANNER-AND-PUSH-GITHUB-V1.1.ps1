...
$ChangedListPath = Join-Path $PSScriptRoot "CHANGED-FILES-GENERAL-NOTIFICATIONS-V1.1.txt"

try {
    Step "Klasorler ve GitHub deposu kontrol ediliyor"

    if (-not (Test-Path -LiteralPath $SourceRoot -PathType Container)) {
        throw "Canli proje klasoru bulunamadi: $SourceRoot"
    }

    if (-not (Test-Path -LiteralPath (Join-Path $SourceRoot "package.json") -PathType Leaf)) {
        throw "Canli projede package.json bulunamadi: $SourceRoot"
    }
    ...
    if (-not (Test-Path -LiteralPath $ChangedListPath -PathType Leaf)) {
        throw "Degisen dosya listesi bulunamadi: $ChangedListPath"
    }
    ...
    try {
        ...
        if ($dirty) {
            throw "C:\Web\burger-github temiz degil. Once mevcut degisiklikleri commit veya stash yap."
        }
    ...
    Step "Eksik qr-scanner paketi canli projeye kuruluyor"
    ...
    try {
        ...
        Step "Genel bildirim regresyon testleri calistiriliyor"
        ...
        if (Test-Path -LiteralPath (Join-Path $SourceRoot "print-proxy\index.cjs")) {
            ...
            Write-Host "Schnellbestellung testi atlandi: print-proxy/index.cjs bulunamadi." -ForegroundColor Yellow
        }
    ...
    Step "Degisen dosyalar GitHub klonuna kopyalaniyor"
    ...
    foreach ($relative in $changedFiles) {
        ...
        if ($extension -in @(".png", ".jpg", ".jpeg", ".webp", ".ico")) {
            continue
        }
    ...
    try {
        ...
        Run-Checked "git" "commit" "-m" "fix: install qr scanner and complete notification system"
    ...
    Write-Host "qr-scanner kuruldu; package.json ve package-lock.json birlikte gonderildi." -ForegroundColor Green
    Write-Host "Vercel deployment tamamlaninca /install ve /admin/notifications test edilsin." -ForegroundColor Yellow
    exit 0
}
catch {
    Write-Host ""
