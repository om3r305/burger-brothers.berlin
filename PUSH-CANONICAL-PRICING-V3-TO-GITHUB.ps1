...
Canonical pricing kök düzeltmesi V3 GitHub gönderimi
...
function Update-CanonicalPricingSecurityTest {
    param([Parameter(Mandatory = $true)][string]$Path)

    if (-not (Test-Path -LiteralPath $Path -PathType Leaf)) {
        throw "Security test dosyası bulunamadı: $Path"
    }

    $source = [System.IO.File]::ReadAllText($Path)
    $marker = "CANONICAL_PRICE_SOURCE_OF_TRUTH_V3"

    if ($source.Contains($marker)) {
        return
    }

    $legacyLine = 'if (!orderPricing.includes("ORDER_PRICE_CHANGED")) failures.push("submitted/canonical price mismatch protection missing");'

    if (-not $source.Contains($legacyLine)) {
        throw @"
Beklenen eski canonical price security assertion bulunamadı.
Dosya otomatik değiştirilmedi:
$Path
"@
    }
...
'@
    ...
    $updated = $source.Replace($legacyLine, $replacement)

    if ($updated -eq $source -or -not $updated.Contains($marker)) {
        throw "Security test canonical source-of-truth güncellemesi uygulanamadı."
    }
...
Write-Step "Canonical pricing security testi yeni source-of-truth mimarisine uyarlanıyor"

Update-CanonicalPricingSecurityTest -Path (
    Join-Path $src "tools\security-tests.mjs"
)
...
$checkoutTypesSource = [System.IO.File]::ReadAllText(
    (Join-Path $src "types\checkout.ts")
)
$securityTestsSource = [System.IO.File]::ReadAllText(
    (Join-Path $src "tools\security-tests.mjs")
)
...
if (-not $checkoutTypesSource.Contains("pricingAdjustment?: PricingAdjustment;")) {
    throw "OrderCreateEnvelope pricingAdjustment tipi güvenli değil."
}

if (-not $checkoutTypesSource.Contains("canonicalPricing?: CanonicalPricingSnapshot;")) {
    throw "OrderCreateEnvelope canonicalPricing tipi güvenli değil."
}

$orderEnvelopeBlock = [System.Text.RegularExpressions.Regex]::Match(
    $checkoutTypesSource,
    "export type OrderCreateEnvelope = \{[\s\S]*?\n\};"
).Value

if (
    $orderEnvelopeBlock.Contains("pricingAdjustment?: unknown;") -or
    $orderEnvelopeBlock.Contains("canonicalPricing?: unknown;")
) {
    throw "OrderCreateEnvelope içinde eski unknown pricing alanları bulundu."
}

if (-not $securityTestsSource.Contains("CANONICAL_PRICE_SOURCE_OF_TRUTH_V3")) {
    throw "Canonical pricing security test marker bulunamadı."
}

if ($securityTestsSource.Contains("submitted/canonical price mismatch protection missing")) {
    throw "Eski ORDER_PRICE_CHANGED security assertion hâlâ mevcut."
}

if (-not $securityTestsSource.Contains("const payableCents = rebuiltPricing.payableCents")) {
    throw "Security test canonical Stripe amount kaynağını doğrulamıyor."
}
...
try {
    ...
    try {
        ...
        Invoke-Native "git" @(
            "commit",
            "-m",
            "fix(pricing): canonicalize checkout totals without blocking v3"
        ...
        Write-Host "CANONICAL PRICING V3 DÜZELTMESİ GITHUB'A GÖNDERİLDİ" -ForegroundColor Green
        Write-Host "============================================================" -ForegroundColor Green
