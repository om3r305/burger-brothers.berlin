...
$ExpectedBranch = 'main'
$CommitMessage = 'feat: add in-app Schnell QR and completed-order lock'

$Stamp = Get-Date -Format 'yyyyMMdd-HHmmss'
$BackupRoot = 'C:\Web\burger-github-backups'
$Backup = Join-Path $BackupRoot ("schnell-qr-tv-lock-v8-" + $Stamp)
...
tools/tv-refactor-regression-tests.cjs
README-SCHNELL-QR-TV-LOCK-V8.md
VERIFY-SCHNELL-QR-TV-LOCK-V8.md
CHANGED-FILES-SCHNELL-QR-TV-LOCK-V8.txt
SHA256SUMS-SCHNELL-QR-TV-LOCK-V8.txt
PUSH-SCHNELL-QR-TV-LOCK-V8-TO-GITHUB.ps1
RUN-SCHNELL-QR-TV-LOCK-V8-GITHUB-PUSH.bat
'@
...
Write-Host ' SCHNELL QR + TV LOCK V8 / TEST / BUILD / COMMIT / GITHUB PUSH' -ForegroundColor Cyan
Write-Host '====================================================================' -ForegroundColor Cyan
Write-Host ''
...
try {
    ...
    Write-Step '[6/10] TypeScript, Schnellbestellung ve TV lock testleri çalışıyor...'
    ...
    Write-Step '[9/10] Yalnız Schnell QR + TV Lock V8 dosyaları stage ediliyor...'
    ...
    if ($Staged.Count -eq 0) {
        Write-Host 'Bütün Schnell QR + TV Lock V8 dosyaları zaten GitHub ile aynı.' -ForegroundColor Green
    ...
    Write-Host ' SCHNELL QR + TV LOCK V8 GITHUB GÖNDERİMİ BAŞARILI' -ForegroundColor Green
    Write-Host '====================================================================' -ForegroundColor Green
    Write-Host "Branch: $Branch" -ForegroundColor Cyan
