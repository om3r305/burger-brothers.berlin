...
export default function SceneListPanel({ document, selectedId, snapshot, onEnabledChange, onAdd, onSelect, onMove, onDuplicate, onDelete }: Props) {
  return <aside className="space-y-4 rounded-2xl border border-stone-800 bg-stone-900/55 p-4">
    <div className="flex items-center justify-between"><div><h2 className="font-black">Sahneler</h2><p className="text-xs text-stone-500">{document.scenes.length} sahne · {CANONICAL_SCENE_TYPES.length} sade tür</p></div><label className="flex items-center gap-2 text-xs text-stone-400">Aktif<input type="checkbox" checked={document.enabled} onChange={(event)=>onEnabledChange(event.target.checked)}/></label></div>
