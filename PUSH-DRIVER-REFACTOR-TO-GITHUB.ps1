...
Driver güvenli refactor GitHub gönderimi

Bu dosya doğrudan C:\Web\burger içine konulup oradan çalıştırılır.
Kaynak proje: C:\Web\burger
GitHub repository: C:\Web\burger-github

git init KULLANMAZ.
Build veya test başarısızsa commit/push YAPMAZ.
#>
...
$files = @(
    ...
    "components\driver\DrinkOrderNotice.tsx"
    "components\driver\DriverCompletionToast.tsx"
    "components\driver\DriverConfirmDialog.tsx"
    "components\driver\DriverHeader.tsx"
    "components\driver\DriverLogin.tsx"
    "components\driver\DriverPaymentBadge.tsx"
    "components\driver\DriverPullIndicator.tsx"
    "components\driver\DriverRouteBar.tsx"
    "components\driver\DriverStatCard.tsx"
    "components\driver\DriverToastViewport.tsx"
    "components\driver\OrderWithDetails.tsx"
    "components\driver\PendingOrderCard.tsx"
    "components\driver\TimeBadge.tsx"
    "hooks\driver\use-driver-auth.ts"
    "hooks\driver\use-driver-clock.ts"
    "hooks\driver\use-driver-feedback.ts"
    ...
    "types\driver.ts"
    "docs\driver-refactor-20260722\README.md"
    "docs\driver-refactor-20260722\CHANGED-FILES.txt"
    "docs\driver-refactor-20260722\VERIFY.txt"
)
...
function Normalize-TextFileEnding {
    ...
    if (-not (Test-Path -LiteralPath $Path -PathType Leaf)) {
        throw "Dosya sonu düzeltilecek kaynak bulunamadı: $Path"
    }
...
if ([string]::IsNullOrWhiteSpace($scriptRootCandidate)) {
    throw "Script çalışma klasörü belirlenemedi. Dosyayı C:\Web\burger içine koyup -File ile çalıştırın."
}

$actualScriptRoot = [System.IO.Path]::GetFullPath($scriptRootCandidate).TrimEnd("\")
...
Write-Step "Teslimat dosyalarının sonundaki gereksiz boşluklar temizleniyor"

foreach ($relativePath in $files) {
    Normalize-TextFileEnding -Path (Join-Path $src $relativePath)
}
...
$backupRoot = Join-Path $env:TEMP ("burger-driver-refactor-repo-backup-" + (Get-Date -Format "yyyyMMdd-HHmmss"))
...
try {
    ...
    try {
        ...
        Write-Step "Driver refactor regresyon testi çalıştırılıyor"
        Invoke-Native -Executable "npm.cmd" -Arguments @("run", "driver:refactor:test") -FailureMessage "Driver refactor testi başarısız"
        ...
        Invoke-Native -Executable "git" -Arguments @(
            "commit",
            "-m",
            "refactor(driver): split page into secure typed modules"
        ...
        Write-Host "DRIVER REFACTOR GITHUB'A GÖNDERİLDİ" -ForegroundColor Green
        Write-Host "============================================================" -ForegroundColor Green
