...
import { planBurgerStudioTemplateOrder } from "@/lib/burger-studio-order-plan";
...
export default function BurgerStudioPage() {
  ...
  const [config, setConfig] = useState<BurgerStudioConfig>(
    createDefaultBurgerStudioConfig(),
  );
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([settingsRaw, catalogRaw]) => {
        ...
        const nextConfig = normalizeBurgerStudioConfig(
        ...
        const firstTemplate = nextConfig.templates.find(
          (item) => item.active,
        );
        if (firstTemplate) {
          setRecipe({
  ...
  const linkedProduct = useMemo(() => {
    ...
    return (
      catalog.find((item) =>
        [item.id, item.sku, item.code, item.name].some(
  ...
  const plan = useMemo(() => {
    ...
    return planBurgerStudioTemplateOrder({
      config,
      template,
      recipe,
  ...
  const hasProtein = plan.selected.some(
    (entry) => entry.ingredient.group === "protein",
  );
  ...
  const visibleIngredients = useMemo(
    () =>
      config.ingredients.filter(
        (item) => item.active && item.group === group,
      ),
  ...
  function setIngredientQty(id: string, nextQty: number) {
    ...
    const qty = Math.max(
      0,
      Math.min(ingredient.max, Math.round(nextQty)),
    );
  ...
  function saveCurrentBurger() {
    if (!config.savedBurgersEnabled || !template) return;
  ...
  function loadSavedBurger(item: SavedBurger) {
    const nextRecipe = normalizeBurgerStudioRecipe(item.recipe);
    const matchingTemplate = nextRecipe.templateId
      ? config.templates.find(
  ...
  function addBurgerToCart() {
    if (!canOrder || !template || !linkedProduct) return;

    const cleanName =
      creationName.trim().slice(0, 50) || template.name || "Mein Burger";
    const itemId = String(
      linkedProduct.id || linkedProduct.sku || linkedProduct.code || "",
    );
    const sku = String(
    ...
    addToCart({
      category: String(linkedProduct.category || "burger"),
      item: {
        id: itemId,
        sku,
        ...
        category: String(linkedProduct.category || "burger"),
        description: `Basis: ${template.name}`,
        imageUrl: linkedProduct.imageUrl || linkedProduct.image,
      },
      qty: 1,
      add: plan.add,
      rm: plan.rm,
      ...
      <main className="min-h-dvh bg-[#070707] p-6 text-white">
        <div className="mx-auto max-w-xl rounded-3xl border border-white/10 bg-white/[0.04] p-8 text-center">
          ...
          <p className="mt-3 text-stone-400">
            Das Burger Studio ist gerade geschlossen.
          </p>
          <Link
            href="/menu"
            className="mt-6 inline-flex rounded-2xl bg-amber-400 px-5 py-3 font-black text-black"
          >
            Zurück zum Menü
          </Link>
        </div>
  ...
  return (
    <main className="min-h-dvh bg-[#070707] pb-32 text-white">
      ...
      <div className="mx-auto max-w-7xl px-3 py-5 sm:px-6 sm:py-8">
        <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm text-stone-400 sm:text-base">
              Baue deinen Burger Schicht für Schicht. Der Warenkorb und der
              Server prüfen die aktuellen Preise deiner Basis und Zutaten
              erneut.
            </p>
          </div>
          <Link
            href="/menu"
            className="rounded-xl border border-white/10 bg-white/[0.04] px-4 py-2 text-sm font-bold text-stone-300"
          >
            ← Menü
          </Link>
        </div>
        ...
        {!config.enabled && preview ? (
          <div className="mb-5 rounded-2xl border border-amber-400/30 bg-amber-400/10 p-4 text-sm text-amber-100">
            Vorschau-Modus: Das Studio ist im Admin noch deaktiviert. Du
            kannst alles ansehen, aber erst nach dem Aktivieren bestellen.
          </div>
        ) : null}

        {!config.templates.some((item) => item.active) ? (
          <div className="mb-5 rounded-2xl border border-rose-400/25 bg-rose-400/10 p-4 text-sm text-rose-100">
            Es ist noch keine aktive Burger-Basis eingerichtet. Bitte zuerst
            im Admin einen Burger wie Farmas, Italian oder Classic als Vorlage
            verbinden.
          </div>
        ) : null}
              ...
              <div className="mb-3 flex items-center justify-between gap-3">
                <div>
                  ...
                  <div className="font-black">
                    Wähle deinen Startburger
                  </div>
                </div>
                <div className="rounded-full bg-amber-400 px-3 py-1 text-sm font-black text-black">
                  {fmt(plan.total)}
                </div>
              </div>
                ...
                {config.templates
                  ...
                  .map((item) => (
                    <button
                      ...
                      className={`min-w-40 rounded-2xl border p-3 text-left ${
                        recipe.templateId === item.id
                    ...
                    >
                      ...
                      <div className="mt-1 line-clamp-2 text-xs text-stone-400">
                        {item.description || "Als Basis verwenden"}
                      </div>
                    </button>
                  ))}
              </div>
          ...
          <section className="space-y-5">
            <div className="rounded-3xl border border-white/10 bg-white/[0.035] p-4 sm:p-5">
              <div className="grid grid-cols-5 gap-1 rounded-2xl bg-black/30 p-1">
                {GROUPS.map((entry) => (
                  <button
                    ...
                    className={`rounded-xl px-1 py-2 text-center text-[10px] font-black sm:text-xs ${
                      group === entry.key
              ...
              <div className="mt-4 space-y-2">
                {visibleIngredients.map((ingredient) => {
                  ...
                  const detail =
                    deltaQty > 0
                      ? `Änderung +${fmt(line?.amount || 0)}`
                      : deltaQty < 0
                        ? ingredient.removeCredit > 0
                          ? `Ersatzgutschrift bis ${fmt(ingredient.removeCredit)}`
                          : "Ohne Preisabzug"
                    ...
                    >
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="text-xs text-stone-500">
                          {detail} · extra {fmt(ingredient.addPrice)} · max. {ingredient.max}
                        </div>
                      </div>
                      <div className="flex items-center gap-2 rounded-xl bg-black/40 p-1">
                        <button
                          type="button"
                          onClick={() =>
                            setIngredientQty(ingredient.id, qty - 1)
                          }
                          className="grid h-9 w-9 place-items-center rounded-lg bg-white/[0.07] text-lg font-black"
                        >
                          −
                        </button>
            ...
            <div className="rounded-3xl border border-white/10 bg-white/[0.035] p-4 sm:p-5">
              ...
              <input
                value={creationName}
                onChange={(event) =>
                  setCreationName(event.target.value.slice(0, 50))
                }
              ...
              <div className="mt-4 space-y-2 text-sm">
                <div className="flex justify-between text-stone-400">
                  <span>{template ? `${template.name} Basis` : "Keine Basis"}</span>
                  <span>{fmt(plan.basePrice)}</span>
                </div>
                {plan.lines
                  .filter((line) => line.deltaQty > 0 && line.amount > 0)
                  .map((line) => (
                    <div
                      key={line.ingredient.id}
                      className="flex justify-between gap-3 text-stone-400"
                    >
                      <span>
                        {line.ingredient.name} +{line.deltaQty}
                      </span>
                ...
                <div className="text-[11px] text-stone-600">
                  Aktive Rabatte/Kampagnen werden im Warenkorb wie bei normalen
                  Burgern berücksichtigt.
                </div>
              </div>

              {!linkedProduct && template ? (
                <div className="mt-3 rounded-xl border border-rose-400/25 bg-rose-400/10 p-3 text-sm text-rose-100">
                  Die verknüpfte Menü-Basis wurde nicht gefunden. Bitte im
                  Admin neu verbinden.
                </div>
              ) : null}
              ...
              {!modeAllowed ? (
                <div className="mt-3 rounded-xl border border-rose-400/25 bg-rose-400/10 p-3 text-sm text-rose-100">
                  Für {orderMode === "delivery" ? "Lieferung" : "Abholung"} ist
                  das Burger Studio aktuell deaktiviert.
                </div>
              ) : null}

              {!hasProtein ? (
                <div className="mt-3 text-xs font-bold text-amber-300">
                  Bitte mindestens ein Protein wählen.
                </div>
              ) : null}
              ...
              <div className="mt-4 grid grid-cols-2 gap-2">
                {config.savedBurgersEnabled ? (
                  <button
                    type="button"
                    onClick={saveCurrentBurger}
                    disabled={!template}
                    className="rounded-2xl border border-white/10 bg-white/[0.05] px-4 py-3 font-black disabled:opacity-35"
                  >
                    ♡ Speichern
                  </button>
                ) : null}
              ...
              <div className="rounded-3xl border border-white/10 bg-white/[0.035] p-4 sm:p-5">
                ...
                <div className="space-y-2">
                  {saved.map((item) => (
                    <div
                      key={item.id}
                      className="flex items-center gap-2 rounded-2xl border border-white/10 bg-black/20 p-3"
                    >
                      <button
                        type="button"
                        onClick={() => loadSavedBurger(item)}
                        className="min-w-0 flex-1 text-left"
                      >
                        <div className="truncate font-bold">{item.name}</div>
                        <div className="text-xs text-stone-500">
                          Mit aktuellen Preisen neu bauen
                        </div>
                      </button>
                      <button
                        type="button"
                        onClick={() => removeSavedBurger(item.id)}
                        className="rounded-lg px-2 py-1 text-stone-500 hover:bg-white/10 hover:text-white"
                      >
                        ✕
                      </button>
                    </div>
      ...
      {toast ? (
        <div className="fixed bottom-24 left-1/2 z-[100] -translate-x-1/2 rounded-full border border-amber-300/30 bg-stone-950 px-5 py-3 text-sm font-black shadow-2xl">
          {toast}
        </div>
      ) : null}
    </main>
