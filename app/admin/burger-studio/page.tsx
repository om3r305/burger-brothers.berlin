...
export default function BurgerStudioAdminPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([settingsRaw, catalogRaw]) => {
        ...
        setConfig(
          normalizeBurgerStudioConfig(settings?.menu?.burgerStudio),
        );
  ...
  function setTemplateIngredient(
    ...
    const qty = Math.max(
      0,
      Math.min(ingredient.max, Math.round(Number(qtyValue) || 0)),
    );
  ...
  async function save() {
    ...
    try {
      const clean = normalizeBurgerStudioConfig({
        ...config,
        // v1 deliberately uses real menu products as canonical bases.
        scratchEnabled: false,
      });

      const activeTemplates = clean.templates.filter((item) => item.active);
      if (clean.enabled && activeTemplates.length === 0) {
        throw new Error(
          "Burger Studio’yu açmadan önce en az bir aktif burger şablonu bağlamalısın.",
        );
      }
      ...
      for (const template of activeTemplates) {
        ...
        const proteinQty = Object.entries(template.recipe).reduce(
          (sum, [ingredientId, qty]) => {
            const ingredient = ingredientMap.get(ingredientId);
            return ingredient?.group === "protein"
              ? sum + Number(qty || 0)
              : sum;
          },
          0,
      ...
      // First synchronize canonical extras into the linked real menu products.
      // If this fails, the public feature is not activated with unsafe pricing.
      ...
      if (!syncResponse.ok || syncBody?.ok === false) {
        throw new Error(
          String(
            syncBody?.message ||
              syncBody?.error ||
              "Burger Studio fiyat extraları ürünlere senkronlanamadı.",
          ),
        );
      }
      ...
      if (!response.ok || body?.ok === false) {
        throw new Error(
          String(
            body?.message ||
              body?.error ||
              "Burger Studio konnte nicht gespeichert werden.",
          ),
        );
      }
      ...
      setConfig(clean);
      setMessage(
        `Burger Studio gespeichert ✓ · ${syncBody?.updated ?? 0} ürünün Studio extraları senkronlandı.`,
      );
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6">
      ...
      {error ? (
        <div className="rounded-2xl border border-rose-400/25 bg-rose-400/10 p-4 text-sm font-bold text-rose-100">
          {error}
        </div>
      ) : null}
      ...
      <section className="grid gap-4 md:grid-cols-3">
        <ToggleCard
          title="Burger Studio"
          description="Müşteri menüsünde Burger Studio girişini gösterir. Varsayılan kapalıdır."
        ...
        <ToggleCard
          title="Lieferung"
          description="Teslimat siparişlerinde Studio kullanılabilir."
      ...
      <section className="rounded-3xl border border-white/10 bg-white/[0.03] p-4 sm:p-6">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div>
            <h2 className="text-xl font-black text-white">Kayıt Kuralları</h2>
            <p className="mt-1 text-sm text-stone-500">
              Kayıtlı burgerler aynı cihazda tutulur; yeniden açıldığında güncel
              ürün ve extra fiyatlarıyla tekrar hesaplanır.
            </p>
          </div>
        </div>
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          <NumberField
            ...
            onChange={(value) =>
              updateConfig({
                maxIngredients: Math.max(1, Math.min(40, Math.round(value))),
              })
            }
          />
          <NumberField
            ...
            onChange={(value) =>
              updateConfig({
                maxSavedBurgers: Math.max(1, Math.min(30, Math.round(value))),
              })
            }
          />
            ...
            <label className="flex cursor-pointer items-center justify-between gap-3">
              <span>
                ...
                <span className="mt-1 block text-xs text-stone-500">
                  Müşteri kendi tarifini daha sonra tekrar açabilir.
                </span>
              </span>
              <input
                type="checkbox"
                ...
                onChange={(event) =>
                  updateConfig({ savedBurgersEnabled: event.target.checked })
                }
      ...
      <section className="rounded-3xl border border-white/10 bg-white/[0.03] p-4 sm:p-6">
        <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
          ...
          <button
            ...
            className="rounded-xl border border-amber-400/30 bg-amber-400/10 px-4 py-2 text-sm font-black text-amber-200"
          >
            + Malzeme
          </button>
        </div>
          ...
          {config.ingredients.map((ingredient) => (
            ...
            >
              ...
              <TextField
                ...
                onChange={(name) => updateIngredient(ingredient.id, { name })}
              />
              ...
              <label className="text-xs font-bold text-stone-400">
                ...
                <select
                  ...
                  onChange={(event) =>
                    updateIngredient(ingredient.id, {
                      group: event.target.value as BurgerStudioGroup,
                    })
                  }
              ...
              <NumberField
                ...
                step="0.1"
                onChange={(value) =>
                  updateIngredient(ingredient.id, { addPrice: money(value) })
                }
              />
              <NumberField
                label="Maks."
                ...
                onChange={(value) =>
                  updateIngredient(ingredient.id, {
                    max: Math.max(1, Math.min(6, Math.round(value))),
                  })
                }
              />
              <button
                ...
                className="h-11 rounded-xl border border-rose-400/20 bg-rose-400/5 px-3 text-sm font-black text-rose-300"
              >
                Sil
              </button>
            </div>
      ...
      <section className="rounded-3xl border border-white/10 bg-white/[0.03] p-4 sm:p-6">
        <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
          <div>
            <h2 className="text-xl font-black text-white">Burger Şablonları</h2>
            <p className="mt-1 max-w-4xl text-sm text-stone-500">
        ...
        {!config.templates.length ? (
          <div className="rounded-2xl border border-dashed border-white/15 p-6 text-center text-sm text-stone-500">
            Henüz şablon yok. “+ Şablon” ile Farmas, Italian, Hit Burger gibi
            ürünleri bağla.
          </div>
        ) : null}
        ...
        <div className="space-y-3">
          {config.templates.map((template) => {
            ...
            const linked = burgerProducts.find(
              (product) => productRef(product) === template.productRef,
            );
              ...
              >
                <div className="flex flex-wrap items-center gap-3 p-4">
                  ...
                  <button
                    type="button"
                    onClick={() =>
                      setOpenTemplateId(open ? null : template.id)
                    }
                  ...
                  >
                    ...
                    <div className="mt-1 truncate text-xs text-stone-500">
                      {linked
                        ? `${linked.name} · ${money(linked.price).toFixed(2)} € güncel baz`
                        : "⚠ Menü ürünü seçilmedi"}
                    </div>
                  </button>
                  <span className="rounded-full border border-white/10 bg-white/[0.04] px-3 py-1 text-xs font-black text-stone-300">
                    {Object.values(template.recipe).reduce(
                  ...
                  <button
                    ...
                    className="rounded-lg px-3 py-2 text-xs font-black text-rose-300 hover:bg-rose-400/10"
                  >
                    Sil
                  </button>
                </div>
                ...
                {open ? (
                  <div className="border-t border-white/10 p-4 sm:p-5">
                    <div className="grid gap-4 lg:grid-cols-3">
                      ...
                      <label className="text-xs font-bold text-stone-400 lg:col-span-2">
                        <span className="mb-1 block">
                          Gerçek menü ürünü / baz fiyat
                        </span>
                        <select
                          value={template.productRef}
                          onChange={(event) =>
                            updateTemplate(template.id, {
                              productRef: event.target.value,
                            })
                          }
                          className="h-11 w-full rounded-xl border border-white/10 bg-stone-950 px-3 text-sm text-white outline-none"
                        >
                          ...
                          {burgerProducts.map((product) => {
                            ...
                            return (
                              <option key={ref} value={ref}>
                                {product.name} — {money(product.price).toFixed(2)} €
                              </option>
                            );
                          })}
                        </select>
                    ...
                    <div className="mt-5">
                      <div className="mb-2 text-xs font-black uppercase tracking-widest text-stone-500">
                        Normal ürün reçetesi
                      </div>
                      <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-3">
                        {config.ingredients
                          ...
                          .map((ingredient) => {
                            const qty = Number(
                              template.recipe[ingredient.id] || 0,
                            );
                            return (
                              <div
                                key={ingredient.id}
                              ...
                              >
                                ...
                                <button
                                  ...
                                  className="grid h-8 w-8 place-items-center rounded-lg bg-white/[0.06] font-black"
                                >
                                  −
                                </button>
                                ...
                                <button
                                  ...
                                  className="grid h-8 w-8 place-items-center rounded-lg bg-amber-400 font-black text-black disabled:opacity-30"
                                >
                                  +
                                </button>
                              </div>
      ...
      <div className="sticky bottom-[calc(env(safe-area-inset-bottom)+5.2rem)] z-20 flex justify-end lg:bottom-4">
        <button
          ...
          onClick={save}
          disabled={saving}
          className="rounded-2xl bg-amber-400 px-6 py-3 font-black text-black shadow-2xl disabled:opacity-50"
        >
          {saving ? "Kaydediliyor…" : "Burger Studio’yu Kaydet"}
        </button>
      </div>
    </div>
  );
  ...
  return (
    <label className="text-xs font-bold text-stone-400">
      ...
      <input
        ...
        className="h-11 w-full rounded-xl border border-white/10 bg-stone-950 px-3 text-sm text-white outline-none focus:border-amber-400/50"
      />
    </label>
  );
}
