...
export default function DriverPage({ params }: { params: { id: string } }) {
  ...
  // ✔ Siparişi DB'den çek (gerekirse LS fallback lib/orders tarafında)
  useEffect(() => {
    let cancelled = false;
    (async () => {
      const o = await fetchOrderFromDb(orderId);
  ...
  async function goUnterwegs() {
    if (!isDelivery || !order) return;
    const dn = driverName.trim();
    if (!dn) {
      if (!confirm("Şoför adı boş bırakıldı. Yine de devam edilsin mi?")) return;
    }

    setBusy(true);
    try {
      // LS meta: şoför adını lokal kopyaya yaz (opsiyonel)
      upsertOrder({
        ...(order as any),
        meta: { ...((order as any).meta || {}), driverName: dn || undefined },
      } as Partial<StoredOrder> as any);

      // Local history & status
      markOrderOutForDelivery(order.id, dn || "driver-qr");

      // DB'ye status yaz
      await persistStatusToDb(order.id, "out_for_delivery", dn || undefined);
  ...
  async function goDone() {
    ...
    try {
      // Local status
      markOrderDone(order.id, driverName.trim() || "driver-qr");

      // DB'ye status yaz
      await persistStatusToDb(order.id, "done", driverName.trim() || undefined);
