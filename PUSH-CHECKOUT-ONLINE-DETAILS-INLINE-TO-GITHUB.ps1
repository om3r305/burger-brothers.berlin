...
Checkout Online-Zahlung ayrıntılarını kartın hemen altına taşıma
GitHub gönderme scripti
...
$files = @(
    ...
    "docs\checkout-online-details-inline-20260722\README.md"
    "docs\checkout-online-details-inline-20260722\CHANGED-FILES.txt"
    "docs\checkout-online-details-inline-20260722\VERIFY.txt"
)
...
$onlineMarker = 'onClick={() => setPaymentMethod("online")}'
$savedMarker = "Gespeicherte Zahlungsart"
$trustMarker = '<PaymentTrustBadges compact className="mt-3" />'
$splitMarker = 'onClick={() => setPaymentMethod("split_contactless")}'

$onlineIndex = $checkoutSource.IndexOf($onlineMarker)
$savedIndex = $checkoutSource.IndexOf($savedMarker, [Math]::Max(0, $onlineIndex))
$trustIndex = $checkoutSource.IndexOf($trustMarker, [Math]::Max(0, $onlineIndex))
$splitIndex = $checkoutSource.IndexOf($splitMarker, [Math]::Max(0, $onlineIndex))
...
if (
    ...
    throw "Checkout ödeme alanı yapısal kontrolü başarısız."
}

if (
    -not (
        $onlineIndex -lt $savedIndex -and
        $savedIndex -lt $trustIndex -and
        $trustIndex -lt $splitIndex
    )
) {
    throw "Online ödeme ayrıntıları beklenen sırada değil."
}
...
$backupRoot = Join-Path $env:TEMP (
    "burger-checkout-layout-backup-" + (Get-Date -Format "yyyyMMdd-HHmmss")
)
...
try {
    ...
    try {
        ...
        $paymentTest = Join-Path $repo "tools\payment-center-architecture-tests.cjs"

        if (Test-Path -LiteralPath $paymentTest -PathType Leaf) {
            Write-Step "Payment Center mimari testi"
            Invoke-Native "node.exe" @(
                "tools/payment-center-architecture-tests.cjs"
            ) "Payment Center mimari testi başarısız"
        }
        ...
        Invoke-Native "git" @(
            "commit",
            "-m",
            "fix(checkout): show online payment details inline"
        ...
        Write-Host "CHECKOUT ÖDEME ALANI GITHUB'A GÖNDERİLDİ" -ForegroundColor Green
        Write-Host "============================================================" -ForegroundColor Green
