...
Driver Karten-App Auswahl + Rota Önizleme — GitHub gönderimi

Bu script doğrudan C:\Web\burger içine konulup oradan çalıştırılır.
...
- Build/test başarısızsa commit veya push yapılmaz.
- Yeni ve takip edilmeyen dosyalarda git restore pathspec kullanılmaz.
#>
...
$files = @(
    ...
    "types\driver.ts"
    "docs\driver-map-chooser-20260722\README.md"
    "docs\driver-map-chooser-20260722\CHANGED-FILES.txt"
    "docs\driver-map-chooser-20260722\VERIFY.txt"
)
...
$actualRoot =
    [System.IO.Path]::GetFullPath($rootCandidate).TrimEnd("\")

if ($actualRoot -ne $expectedRoot) {
    throw @"
Bu script doğrudan C:\Web\burger içine konulup oradan çalıştırılmalıdır.
...
if (-not (Test-Path -LiteralPath (Join-Path $repo ".git") -PathType Container)) {
    throw @"
C:\Web\burger-github geçerli Git repository değil.
git init kesinlikle kullanılmayacak.
"@
}

Write-Step "Teslimat kaynakları ve güvenlik politikası kontrol ediliyor"
...
if ($repoStatus.Count -gt 0) {
    Write-Host "Repository içinde önceden kalan değişiklikler var:" `
...
$backupRoot = Join-Path `
    $env:TEMP `
    ("burger-driver-map-backup-" + (Get-Date -Format "yyyyMMdd-HHmmss"))
...
try {
    ...
    try {
        if (
            -not (
                Test-Path `
                    -LiteralPath (Join-Path $repo "node_modules") `
                    -PathType Container
            )
        ) {
        ...
        Invoke-Native `
            "npm.cmd" `
            @("run", "driver:refactor:test") `
            "Driver regresyon testi başarısız"
        ...
        if ($LASTEXITCODE -eq 0) {
            Write-Host `
                "Gönderilecek yeni değişiklik yok. Commit/push yapılmadı." `
                -ForegroundColor Yellow
            return
        }
        ...
        Invoke-Native `
            ...
            @(
                "commit"
                "-m"
                "feat(driver): remember map app and open route previews"
            ) `
        ...
        Write-Host "DRIVER HARİTA SEÇİMİ GITHUB'A GÖNDERİLDİ" `
            -ForegroundColor Green
        Write-Host "============================================================" `
            -ForegroundColor Green
    ...
    if (-not $commitCreated) {
        ...
        Restore-RepositoryFiles `
            -BackupRoot $backupRoot `
            -ExistedMap $existedMap
    }
    else {
        Write-Host @"
Commit oluşturuldu fakat push tamamlanamadı.
Commit korunmuştur; bağlantı düzeldikten sonra push tekrar denenebilir.
"@ -ForegroundColor Yellow
    }
