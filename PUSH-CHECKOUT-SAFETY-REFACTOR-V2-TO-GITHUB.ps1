...
Checkout güvenli tip / toast / accessibility refactor V2 GitHub gönderimi
...
$files = @(
    ...
    "components\store.ts"
    "components\checkout\CheckoutToastViewport.tsx"
    "lib\checkout\runtime.ts"
    "lib\settings.ts"
    "types\checkout.ts"
    "tools\checkout-safety-regression-tests.cjs"
    "package.json"
    "docs\checkout-safety-refactor-20260722\README.md"
    "docs\checkout-safety-refactor-20260722\CHANGED-FILES.txt"
    "docs\checkout-safety-refactor-20260722\VERIFY.txt"
)
...
$storeSource = [System.IO.File]::ReadAllText(
    (Join-Path $src "components\store.ts")
)

if ($checkoutSource -match "\balert\s*\(") {
    throw "Checkout içinde native alert bulundu."
}
if ($checkoutSource -match "\bany\b") {
    throw "Checkout içinde explicit any bulundu."
}
if ($storeSource -match "\bany\b") {
    throw "Cart store içinde explicit any bulundu."
}
if (-not $checkoutSource.Contains("parsePaymentPrepareResponse")) {
    throw "Payment response runtime parser bağlantısı bulunamadı."
}
if (-not $checkoutSource.Contains("<CheckoutToastViewport")) {
    throw "Checkout toast bağlantısı bulunamadı."
}
if (-not $checkoutSource.Contains("const selectedValue: unknown = payments[key]")) {
    throw "Opsiyonel ödeme ayarı için güvenli type narrowing bulunamadı."
}
if (-not $checkoutSource.Contains("isRecord(selectedValue) ? selectedValue.enabled : undefined")) {
    throw "Ödeme enabled alanı güvenli biçimde okunmuyor."
}
...
$backupRoot = Join-Path $env:TEMP (
    "burger-checkout-safety-backup-" + (Get-Date -Format "yyyyMMdd-HHmmss")
)
...
try {
    ...
    try {
        ...
        Write-Step "Checkout güvenlik/regresyon testi"
        Invoke-Native "npm.cmd" @("run", "checkout:safety:test") "checkout:safety:test başarısız"

        Write-Step "Bütün güvenlik regresyon testleri"
        ...
        Invoke-Native "git" @(
            "commit",
            "-m",
            "refactor(checkout): harden types feedback and accessibility v2"
        ...
        Write-Host "CHECKOUT GÜVENLİ REFACTOR V2 GITHUB'A GÖNDERİLDİ" -ForegroundColor Green
        Write-Host "============================================================" -ForegroundColor Green
