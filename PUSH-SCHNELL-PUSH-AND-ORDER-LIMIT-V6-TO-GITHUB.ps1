...
$ExpectedBranch = 'main'
$CommitMessage = 'feat: add Android background ready push and active-order limits'

$Stamp = Get-Date -Format 'yyyyMMdd-HHmmss'
$BackupRoot = 'C:\Web\burger-github-backups'
$Backup = Join-Path $BackupRoot ("schnell-push-order-limit-v6-" + $Stamp)
...
package.json
public/sw.js
tools/generate-schnell-vapid.mjs
tools/schnellbestellung-regression-tests.cjs
README-SCHNELL-PUSH-AND-ORDER-LIMIT-V6.md
VERIFY-SCHNELL-PUSH-AND-ORDER-LIMIT-V6.md
CHANGED-FILES-SCHNELL-PUSH-AND-ORDER-LIMIT-V6.txt
SHA256SUMS-SCHNELL-PUSH-AND-ORDER-LIMIT-V6.txt
PUSH-SCHNELL-PUSH-AND-ORDER-LIMIT-V6-TO-GITHUB.ps1
...
Write-Host ' SCHNELL PUSH + ORDER LIMIT V6 / TEST / BUILD / COMMIT / GITHUB PUSH' -ForegroundColor Cyan
Write-Host '====================================================================' -ForegroundColor Cyan
Write-Host ''
...
try {
    ...
    Write-Step '[9/10] Yalnız Variant + Ready V5 dosyaları stage ediliyor...'
    ...
    if ($Staged.Count -eq 0) {
        Write-Host 'Bütün Variant + Ready V5 dosyaları zaten GitHub ile aynı.' -ForegroundColor Green
    ...
    Write-Host ' SCHNELL PUSH + ORDER LIMIT V6 GITHUB GÖNDERİMİ BAŞARILI' -ForegroundColor Green
    Write-Host '====================================================================' -ForegroundColor Green
    Write-Host "Branch: $Branch" -ForegroundColor Cyan
