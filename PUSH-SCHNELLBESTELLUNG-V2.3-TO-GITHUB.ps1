...
$ExpectedBranch = 'main'
$CommitMessage = 'fix: normalize dine-in TV orders and speed Schnellbestellung catalog'

$Stamp = Get-Date -Format 'yyyyMMdd-HHmmss'
$BackupRoot = 'C:\Web\burger-github-backups'
$Backup = Join-Path $BackupRoot ("schnellbestellung-v2-3-" + $Stamp)
...
types/tv.ts
README-SCHNELLBESTELLUNG-V2.3.md
VERIFY-SCHNELLBESTELLUNG-V2.3.md
CHANGED-FILES-SCHNELLBESTELLUNG-V2.3.txt
SHA256SUMS-SCHNELLBESTELLUNG-V2.3.txt
PUSH-SCHNELLBESTELLUNG-V2.3-TO-GITHUB.ps1
...
Write-Host ' SCHNELLBESTELLUNG V2.3 / TEST / BUILD / COMMIT / GITHUB PUSH' -ForegroundColor Cyan
Write-Host '====================================================================' -ForegroundColor Cyan
Write-Host ''
...
if (-not (Test-Path -LiteralPath (Join-Path $Repo '.git') -PathType Container)) {
    throw "GitHub klasöründe .git bulunamadı. Script repository oluşturmaz."
}
...
if ($DirtyBefore.Count -gt 0) {
    Write-Host 'C:\Web\burger-github temiz değil:' -ForegroundColor Red
    $DirtyBefore | ForEach-Object { Write-Host $_ -ForegroundColor Red }
    throw 'Mevcut repository değişikliklerine dokunulmadı.'
}

Write-Step '[1/10] Remote main kontrol ediliyor...'
...
try {
    ...
    Write-Step '[8/10] Production build çalışıyor...'
    ...
    if ($UnexpectedPaths.Count -gt 0) {
        Write-Host 'Build/test beklenmeyen dosyaları değiştirdi:' -ForegroundColor Red
    ...
    Write-Step '[9/10] Yalnız V2.3 dosyaları stage ediliyor...'
    ...
    if ($Staged.Count -eq 0) {
        Write-Host 'Bütün V2.3 dosyaları zaten GitHub ile aynı.' -ForegroundColor Green
    ...
    Write-Host ' SCHNELLBESTELLUNG V2.3 GITHUB GÖNDERİMİ BAŞARILI' -ForegroundColor Green
    Write-Host '====================================================================' -ForegroundColor Green
    Write-Host "Branch: $Branch" -ForegroundColor Cyan
