...
async function main() {
  ...
  // Session/authorization checks must not depend on the live restaurant's
  // Shop-Status. The maintenance gate has dedicated coverage elsewhere.
  global.fetch = async () =>
    new Response(
      JSON.stringify({
        ok: true,
  ...
  const { middleware } = require(path.join(root, "middleware.ts"));
