...
const admin = [
  fs.readFileSync('app/admin/showcase/page.tsx', 'utf8'),
  fs.readFileSync('components/showcase/admin/SceneBasicsEditor.tsx', 'utf8'),
].join('\n');
...
assert(admin.includes('<Field label="Başlık"'), 'Admin başlık alanı eksik');
assert(admin.includes('<Field label="Alt başlık"'), 'Admin alt başlık alanı eksik');
assert(admin.includes('label={scene.type === "message" ? "Duyuru metni" : "Ek metin"}'), 'Duyuru alan etiketi eksik');
